...
const LevelData l1 = { LEVEL_0_1, 0, BACKGROUND, {TILE_ER, TILE_F_IN, TILE_F, TILE_GR}, {32*8, 40*8}, {30,5},    (sizeof(LEVEL_0_1)>>2) };
const LevelData l2 = { LEVEL_0_2, 1, BACKGROUND, {TILE_ER, TILE_F_IN, TILE_F, TILE_GR}, {32*8, 40*8}, {100,100}, (sizeof(LEVEL_0_2)>>2) };//can not end level
...
const LevelData *levelList[2] = {&l1, &l2};
...
void movePlayer(){
  ...
  if(((Ppos[0])>>3) > (levelX+(10<<3))){
    ...
    if(levelX > (currentLevel.lengthOfLevel<<3)-128 ){levelX = (currentLevel.lengthOfLevel<<3)-128;}//don't move past the end of the level (128 for the x dimension of the screen in pixels)
  }else
  ...
  for(i=0; i<SCREEN_WIDTH+1; i++){//the plus one is to avoid any seams along the screen boundry
    ...
    for(j=0; j<SCREEN_HEIGHT; j++){
      ...
      if(tileIndex){//==1
        ...
        drawTile(i, j, pixelOffset, currentLevel.tileArray[tileIndex>>(j<<1)]);
